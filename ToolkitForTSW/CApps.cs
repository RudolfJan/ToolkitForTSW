  ...
  public static class CApps
    ...
    public static String CompileInstallerScript(String InputFile, String InstallerOptions = "")
      ...
      if (!File.Exists(InstallerName))
        {
        return Log.Trace("Inno Setup not found, expected " + InstallerName, LogEventType.Message);
        }
      ...
      try
        ...
        {
        return Log.Trace("Error creating installer " + E.Message, LogEventType.Error);
        }
      }
    ...
    public static String OpenGenericFile(String Filepath)
      ...
      try
        ...
        if (Filepath.Contains("\'"))
          ...
          return Log.Trace("Cannot open file " + Filepath + " because it contains single quotes. Remove the qute from the file path", LogEventType.Error);
          }
      ...
      if (File.Exists(FilePath))
        ...
        using (var EditScriptProcess = new Process())
          ...
          try
            ...
            {
            return Log.Trace("Cannot open zip file " + FilePath + " reason: " + E.Message, LogEventType.Error);
            }
          }
        }
      ...
      if (File.Exists(Archive))
        ...
        using (var MyProcess = new Process())
          ...
          try
            ...
            if (Stderr.Length > 0)
              {
              return Log.Trace("Error extracting compressed file " + Archive + " message: " + Stderr,
                  LogEventType.Message);
              }
      ...
      if (File.Exists(Archive))
        ...
        using (var MyProcess = new Process())
          ...
          try
            ...
            if (Stderr.Length > 0)
              {
              return Log.Trace("Error extracting compressed files " + Archive + " message: " + Stderr,
                  LogEventType.Message);
              }
    ...
    public static String SevenZipAddAll(String Archive, String InputDirectory, Boolean Recursive = false)
      ...
      using (var MyProcess = new Process())
        ...
        try
          ...
          {
          return Log.Trace("Cannot extract compressed file " + Archive + " reason: " + E.Message, LogEventType.Error);
          }
        }
      }
    ...
    public static String SevenZipAddFiles(String Archive, String InputFiles, Boolean Recursive = false)
      ...
      using (var MyProcess = new Process())
        ...
        try
          ...
          if (Stderr.Length > 0)
            {
            return Log.Trace("Error compressing files " + Archive + " message: " + Stderr,
                LogEventType.Message);
            }
          ...
          {
          return Log.Trace("Cannot compress files " + Archive + " reason: " + E.Message, LogEventType.Error);
          }
        }
      }
      ...
      if (File.Exists(Archive))
        ...
        using (var MyProcess = new Process())
          ...
          try
            ...
            {
            return Log.Trace("Cannot extract compressed files from " + Archive + " reason: " + E.Message,
                LogEventType.Error);
            }
          }
        }
      ...
      if (File.Exists(FilePath))
        ...
        using (var MyProcess = new Process())
          ...
          try
            ...
            if (Stderr.Length > 0)
              {
              return Log.Trace("Error list zip file " + FilePath + " message: " + Stderr, LogEventType.Message);
              }
            ...
            {
            return Log.Trace("Cannot list zip file " + FilePath + " reason: " + E.Message, LogEventType.Error);
            }
          }
        }
...
*/
    ...
    public static String UnpackDirectory(String ArchivePath, String InputDirectory, String OutputDirectory)
      ...
      if (!File.Exists(ArchivePath))
        {
        Result += Log.Trace("Archive " + ArchivePath + " not found", LogEventType.Message); // Cannot open archive
        return Result;
        }
      try
        {
        ...
        {
        Log.Trace("Problem extracting files " + InputDirectory + " because " + E.Message, LogEventType.Error);
        return Result;
        }
      return Result;
      }
