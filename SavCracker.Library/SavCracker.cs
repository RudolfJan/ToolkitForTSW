...
namespace SavCracker.Library
  ...
  public class SavCracker
    ...
    private static byte[] ReadDataFromStream(string scenarioFileName)
      {
      try
        {
    ...
    private SavPropertyModel ProcessNextProperty(int position)
      ...
      if (String.CompareOrdinal(s, "NameProperty") == 0)
        {
        return ExtractAnonymousNameProperty(position);
        }
      else
        {
        return GrabProperty(position, s);
        }
      }

    private SavPropertyModel GrabProperty(int position, string s)
      {
      var element = GrabStringElement();
    ...
    private SavPropertyModel ExtractAnonymousNameProperty(int position)
      {
    ...
    public static PropertyTypeEnum GetArrayElementType(string propertyTypeString)
      {
      switch (propertyTypeString)
        {
        case "NameProperty":
    ...
    private static StructureTypeEnum GetStructureType(string structureTypeTypeString)
      {
    ...
    public static string GetStringFromElement(SavElementModel element)
      {
